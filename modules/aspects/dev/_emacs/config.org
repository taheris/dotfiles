...
(setq-default history-length 1000            ; remember more minibuffer history
              window-combination-resize t    ; resize windows proportionally on split
              x-stretch-cursor t)            ; stretch cursor to cover wide characters (e.g. tab)

(setq auto-save-default t                    ; auto-save files to recover from crashes
      confirm-kill-emacs nil                 ; quit without prompting
      doom-localleader-key ","               ; use comma as localleader
      evil-want-fine-undo t                  ; more granular undo steps in insert mode
      global-auto-revert-non-file-buffers t  ; also revert dired and other non-file buffers
      make-pointer-invisible t               ; hide mouse pointer while typing
      mouse-highlight nil                    ; disable mouse-over highlighting
      scroll-margin 2                        ; keep 2 lines of context above/below cursor
      truncate-string-ellipsis "…"           ; use unicode ellipsis instead of "..."
      undo-limit 10485760)                   ; 10MB undo history
#+end_src
...
Home Manager tangles this file before running =doom sync=, so don't enable Doom's
=:config literate= module as well.
...
Stop Doom from spawning a fresh =#N= workspace for every new client frame.

#+begin_src emacs-lisp
(advice-add '+workspaces-associate-frame-fn :override #'ignore)

(setq server-client-instructions nil)
#+end_src

Open new frames in the last workspace.

#+begin_src emacs-lisp :tangle (if IS-LINUX "yes" "no")
(defvar my/ws-state-file
  (expand-file-name "emacs-active-workspace"
                    (or (getenv "XDG_RUNTIME_DIR") temporary-file-directory)))

(defun my/save-active-workspace (&rest _)
  (run-at-time 0 nil
    (lambda ()
      (when (and (display-graphic-p) (frame-focus-state))
        (with-temp-file my/ws-state-file
          (insert (or (+workspace-current-name) "")))))))

(defun my/restore-workspace-on-frame ()
  (when (display-graphic-p)
    (when-let ((ws (ignore-errors
                     (with-temp-buffer
                       (insert-file-contents my/ws-state-file)
                       (string-trim (buffer-string))))))
      ;; Gate on workspace existence — calling +workspace-switch before persp
      ;; has restored state can land on a prompt path whose (throw 'exit ...)
      ;; escapes frame setup and aborts the new client.
      (when (and (not (string-empty-p ws))
                 (member ws (+workspace-list-names)))
        (+workspace-switch ws)))
    (+workspace/display)))

(add-function :after after-focus-change-function #'my/save-active-workspace)
(add-hook 'persp-activated-functions #'my/save-active-workspace)
(add-hook 'server-after-make-frame-hook #'my/restore-workspace-on-frame)
#+end_src
...
#+begin_src emacs-lisp
(set-popup-rules!
 '(("\\*doom:scratch\\*" :size 0.4 :select t :quit nil)
   ("\\*helpful-*" :size 0.4 :select t)
   ("\\*Flycheck errors\\*" :size 0.4 :select t)
   ("\\*lsp-help\\*" :size 0.4 :select t)
   ("\\*cargo-*" :size 0.4 :select t)
   ("\\*compilation\\*" :size 0.4 :select t)
   ("\\*documentation\\*" :size 0.4 :select t)
   ("\\*calibredb-entry\\*" :size 0.5 :select t)))

(defun +popup/to-frame ()
  "Pop current buffer out to a new frame."
  (interactive)
...
(map! :leader
      :desc "Pop out to frame" "w p" #'+popup/to-frame)
#+end_src
...
Point ~pdf-tools~ to the Nix-provided =epdfinfo= binary, since Doom's
straight.el build doesn't compile the C server. This must run from =init.el=,
before Doom's PDF module restores any PDF buffers.

#+begin_src emacs-lisp :tangle "init.el" :comments no
(setq pdf-info-epdfinfo-program
      (expand-file-name "~/.nix-profile/bin/epdfinfo"))
#+end_src
...
(after! which-key
  (dolist (replacement '((("" . "\\`+?evil[-:/]?\\(?:a-\\)?\\(.*\\)") . (nil . "◂\\1"))
...
(package! ob-mermaid
  ...
  :pin "30c2da02e3d24dbec0d004d3c6dfe7b381500b05")
#+end_src
...
(package! org-srs
  ...
  :pin "e6e5fbfcb8beb520141edac647ccb76af9b71df6")
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el
(package! calibredb :pin "af6fc49edf72f0ebc42d9d3983ad00c268541092")
#+end_src
...
(package! eaf
  ...
  :pin "d0ee517b247fa3c6d1d98caaa00d33fee7f33b1f")
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el
(package! lsp-ltex :pin "f02a9a92202a03635c6cec95ebfff7e21dcc5b38")
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el
(package! gptel :pin "8701e2bd80c5d2091ce2decef5d34d6fce4a3ada")
#+end_src

#+begin_src emacs-lisp
(defun get-llama-cpp-models ()
  "Fetch the models available from the local llama.cpp server."
  (split-string
   (shell-command-to-string
    "curl --fail --silent http://localhost:8080/v1/models | jq --raw-output '.data[].id'")
   "\n" t))
...
(after! gptel
  (let* ((gptel-anthropic (gptel-make-anthropic "Anthropic"
         ...
         (gptel-llama-cpp (gptel-make-openai "llama.cpp"
                            :host "localhost:8080"
                            :protocol "http"
                            :models (get-llama-cpp-models)
                            :stream t)))
...
(package! acp
  ...
  :pin "5140f4121156707245567b8fc3072d4c3b5c867f")
(package! agent-shell
  :recipe (:host github :repo "xenodium/agent-shell")
  :pin "ec1e6a09d04e3f51530ba9c9cf9f7331dfa9f3f4")
#+end_src
...
(after! lsp
  (setq +lsp-defer-shutdown 60)

  (dolist (directory '("[/\\\\]\\.cargo\\'"
                       "[/\\\\]\\.claude\\'"))
...
(after! rust-ts-mode
  ...
  (defun +rust/cargo-clippy ()
    "Run cargo clippy."
    (interactive)
    (compile "cargo clippy --benches --tests --all-features -- -D warnings -D rust-2018-idioms"))
  ...
  (defun +rust/cargo-current-test ()
    "Run the test at point."
    (interactive)
    (compile (format "cargo test --all-features %s" (thing-at-point 'symbol))))
  ...
  (map! :localleader
        ...
        :desc "Cargo Clippy"        "c" #'+rust/cargo-clippy
        :desc "Cargo Format"        "f" #'+rust/cargo-fmt
        :desc "Cargo Check"         "k" #'+rust/cargo-check

        (:prefix ("t" . "cargo test")
         :desc "run tests"     "t" #'+rust/cargo-test
         :desc "run last test" "l" #'recompile
         :desc "test current"  "c" #'+rust/cargo-current-test)
...
(package! protobuf-mode
  ...
  :pin "6d5557a46fdc7d1463fd924f4ed2d028bf189809")
#+end_src
...
(package! move-mode
  ...
  :pin "e72e2d5102669b82ba70599fb64e64241d49376d")
#+end_src
...
#+begin_src emacs-lisp :tangle packages.el
;; typst-ts-mode's autoloads define a compilation mode using this macro.
(require 'compile)

(package! typst-ts-mode
  :recipe (:host codeberg :repo "meow_king/typst-ts-mode")
  :pin "155bb36cff3afe701a0f6b57bd3fe5c9effaa314")
...
(package! ox-typst
  ...
  :pin "3e499609a201405a6064144792dab14e3cc19b93")
#+end_src
